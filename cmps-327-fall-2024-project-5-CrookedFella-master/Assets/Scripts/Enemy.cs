...
public class Enemy : MonoBehaviour
{
    ...
    {
        if (Math.Sqrt(Math.Pow(playerGameObject.GetComponent<Player>().currentTile.indexX - currentTile.indexX, 2) + Math.Pow(playerGameObject.GetComponent<Player>().currentTile.indexY - currentTile.indexY, 2)) < visionDistance){
            state = EnemyState.CHASE;
        }
        else if(Math.Sqrt(Math.Pow(playerGameObject.GetComponent<Player>().currentTile.indexX - currentTile.indexX, 2) + Math.Pow(playerGameObject.GetComponent<Player>().currentTile.indexY - currentTile.indexY, 2)) < visionDistance + 4)
        {
            state = EnemyState.DEFAULT;
        }
        switch (state)
    ...
    private void HandleEnemyBehavior3()
    {
        ...
        switch (state)
        {
            ...
            case EnemyState.MOVING:
                ...
                break;
            case EnemyState.CHASE:
                path = pathFinder.FindPathAStar(currentTile, findClosestTileTwoAway(playerGameObject.GetComponent<Player>().currentTile.Adjacents));
                targetTile = path.Dequeue();
                //move
                velocity = targetTile.transform.position - transform.position;
                transform.position = transform.position + (velocity.normalized * speed) * Time.deltaTime;
    ...
    private Tile findClosestTileTwoAway(List<Tile> Adjacents)
    {
        Tile firstClosestTile = null;
        foreach(Tile tile in Adjacents)
        {
            if(firstClosestTile == null || (Math.Sqrt(Math.Pow(firstClosestTile.indexX - currentTile.indexX, 2) + Math.Pow(firstClosestTile.indexY - currentTile.indexY, 2))) > (Math.Sqrt(Math.Pow(firstClosestTile.indexX - tile.indexX, 2) + Math.Pow(firstClosestTile.indexY - tile.indexY, 2))))
            {
                firstClosestTile = tile;
            }
        }
        Tile realClosestTile = null;
        foreach(Tile tile in firstClosestTile.Adjacents)
        {
            if (realClosestTile == null || (Math.Sqrt(Math.Pow(firstClosestTile.indexX - currentTile.indexX, 2) + Math.Pow(firstClosestTile.indexY - currentTile.indexY, 2))) > (Math.Sqrt(Math.Pow(firstClosestTile.indexX - tile.indexX, 2) + Math.Pow(firstClosestTile.indexY - tile.indexY, 2))))
            {
                realClosestTile = tile;
            }
        }
        return realClosestTile;
    }
}